...
def main():
    ...
    with st.sidebar:
        ...
        if st.button(" Start a new conversation (clear memory)"):
    ...
    with st.chat_message("assistant"):
        ...
        if result["used_fallback"] and not retrieval_result["is_confident"]:
            st.warning(" No strong match found in the archive - escalated to a human agent.")
        elif result["used_fallback"]:
            st.info(" Answer merged directly from the archive (no LLM API key set).")
